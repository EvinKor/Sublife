...
export default function AIPoliciesPage() {
  ...
  const loadPolicies = useCallback(async () => {
    ...
    try {
      const data = await apiClient.get<Array<{
        id: number; policy_key: string; name: string; description: string;
        policy_type: string; value: unknown; is_active: boolean; priority: number;
        evaluation_count: number; last_evaluated_at: string | null;
        updated_at: string | null;
      }>>('/api/service-desk/policies')

      // Map backend format to the Policy type expected by existing UI components
      const mapped: Policy[] = data.map((p) => ({
        id: p.policy_key,
        name: p.name,
        description: p.description || '',
        natural_language: typeof p.value === 'object' ? JSON.stringify(p.value, null, 2) : String(p.value),
        summary: p.description || p.name,
        policy_type: 'logical' as const,
        dsl: { conditions: [], actions: [], match_mode: 'all' as const },
        refined_instruction: null,
        ai_instruction: `Policy type: ${p.policy_type}`,
        entity_name: p.policy_type,
        is_active: p.is_active,
        priority: p.priority,
        tags: [p.policy_type, 'service-desk'],
        execution_count: p.evaluation_count,
      ...
      console.error('[Policies] Failed to load from API:', err)
      // Fallback to demo data if backend not yet running
  ...
  const togglePolicyStatus = useCallback(async (id: string, isActive: boolean) => {
    try {
      await apiClient.put(`/api/service-desk/policies/${id}`, { is_active: !isActive })
